...
#---Main
def pwGenerator(size = 8): # Функция принимает аргумент size, который по умолчанию установлен на 8.
    data = RandPass(size)
    new_password = data[0]
    pw_strength = data[1]
    pw_color = data[2]
...
gui.config(bg = '#1A1A1A') #темный фон
...
gui.resizable(False, False) #---Отключение возможности изменения размеров окна по горизонтали и вертикали.
gui.mainloop()
